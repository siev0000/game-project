...
- 連続メッシュのボーン列に含まれるボーンで、コマ別の手画像／モーフ画像が選択されている場合、その差分画像を別DOM画像ではなく同じメッシュCanvasへ合成する共通処理を追加。右腕・左腕の範囲を手ボーンまで指定すれば、選択した手形がコマごとに腕メッシュと一体描画され、オニオンスキンとスプライトZIPにも同じ結果が入る。持ち物は従来どおり別処理。
