...
async function npcDataGet(partys) {
  ...
  for (const party of partys) {
    ...
    if (npcData) {
      ...
      for (let i = 1; i <= 20; i++) {
        ...
        if (npcData[classKey]) {
          Role.push(await splitClassName(npcData[classKey]));
        }
      }

     
      const { equipmentSlot, inventory } = await setupEquipmentAndInventory(npcData);
      const combinedItems = [...Object.values(equipmentSlot), ...inventory].filter((item) => item !== null);
      const intItem = await findByNameInLists(combinedItems, itemList, equipment);
      ...
      const stats = await calculateStatsAndAbilities2({ Role: Role }, equipmentStats)
      ...
      // NPCデータが見つからない場合の処理
      const intItem = await findByIdInLists(party.inventory, itemList, equipment);
      ...
      const stats = await calculateStatsAndAbilities2({ Role: party.Role }, equipmentStats)
...
//経験値処理
function processExperience(partys, expToAdd) {
...
// Positionの割り当て処理
function assignPosition(preferredType, positionSlots) {
  // 前衛・後衛スロットのチェック
  const types = ["前衛", "後衛"];
...
// 装備の数値を合計する
function calculateEquipmentStats(equipmentSlot, ...lists) {
  // ステータスの初期化
...
// クラス名を分解する関数
async function splitClassName(className) {
  // 正規表現で "Lv" と "Ef" の値を抽出
...
// 装備とインベントリを設定する関数
async function setupEquipmentAndInventory(npcData) {
  // 装備スロットを初期化
  const equipmentSlot = {
    武器: npcData["武器"] || null,
...
async function calculateStatsAndAbilities2(npcData , equipmentStats) {
  ...
  let abilities = await getAvailableAbilities(abilitie);
...
// 敵データの作成
async function generateEnemyData(enemyName) {
...
//パッシブアビリティの合計値を取得
function calculatePassiveStats(abilities) {
  // ステータスの初期化
  const passiveStats = {};
...
// 消費値を作成する
function generateConsumptionStats(party, statKeys, setSumKeys) {
...
// 消費値を作成する
function generateConsumptionEnemyStats(stats, statKeys, setSumKeys) {
  // statKeys と setSumKeys を統合し、重複しないキーのセットを作成
  const allKeys = Array.from(new Set([...statKeys, ...setSumKeys]));
...
// 次の経験値
function getExperience(type, Lv, maxLevel = 60) {
  if (Lv < 1 || Lv > maxLevel) {
...
function generateExperienceTable(type, Lv ,maxLevel=40) {
...
// スロット占有処理
function occupySlot(type, slot) {
  positionSlots[type][slot - 1] = true;
}

// スロット解放処理
function freeSlot(type, slot) {
