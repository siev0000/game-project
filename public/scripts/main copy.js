...
async function loadOptions() {
    try {
        ...
        raceData = classes.filter(cls => ['人族', '亜人', '魔族'].includes(cls['分類'])).filter(isAllConditionsEmptyOrZero);
        classData = classes.filter(cls => cls['分類'] === '職業').filter(isAllConditionsEmptyOrZero);
        ...
        if (['HP', '攻撃', '速度'].includes(stat)) {
            if (sizValue !== 0) { // SIZが0の場合は補正をスキップ
                if (stat === '速度') {
                    ...
                    adjustedValue = Math.round(baseValue + (baseValue * modifier / 100));
                    if (modifier !== 0) {
                        modifierDisplay = ` (${baseValue} ${modifier < 0 ? '' : '+'}${Math.round(modifier)}%)`;
                    }
                }
            }
        }
...
function updateSelectionDisplay() {
    selectedRaceDisplay.textContent = selectedRace ? `種族: ${selectedRace['名前']}` : '種族未選択';
    selectedClassDisplay.textContent = selectedClass ? `クラス: ${selectedClass['名前']}` : 'クラス未選択';
...
function calculateAndDisplayStatus() {
    ...
    statKeys.forEach(key => {
        const raceStat = Math.floor((selectedRace[key] || 0) * (raceLevelEffective + 5) / 10);
        const classStat = Math.floor((selectedClass[key] || 0) * currentClassLevel / 10);
    ...
    // 最大値と2番目の値を特定
    const sortedResults = [...statResults].sort((a, b) => b.totalStat - a.totalStat);
    ...
    console.log('ステータス計算完了', { totalStatus, currentClassLevel, currentRaceLevel, sizModifier });
}
...
function displayTechniques() {
    ...
    TechniqueKeys.forEach(key => {
        const raceTechnique = Math.floor((selectedRace[key] || 0) * (raceLevelEffective + 5) / 10);
        const classTechnique = Math.floor((selectedClass[key] || 0) * currentClassLevel / 10);
...
function getAbilities(data, level) {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        // JSONデータから詳細情報を検索
        const detailedSkill = SkillData.find(Skill => Skill.名前 === SkillName);
        if (detailedSkill) {
            abilities.push({
                name: detailedSkill.名前 || '不明',
                system: detailedSkill.系統 || '-',
                attribute: detailedSkill.分類 || '-',
                description: detailedSkill.説明 || '説明なし'
            });
        }
...
function calculateStats(entity, level) {
    ...
    for (let i = 1; i <= level; i++) {
        ...
        if (entity[SkillKey]) { // entityの該当プロパティが存在するかチェック
...
function setCharacterRole(selectedRace, selectedClass, raceLevel, classLevel) {
    ...
    try {
        ...
        // プレイヤーデータをリセット（必要に応じて）
        playerData = JSON.parse(JSON.stringify(characterData));
        console.log('リセット後のプレイヤーデータ (playerData):', playerData);

        // 種族の分類（例: 人族, 亜人, 魔族）に基づいて初期金額を設定
        const initialMoney = getInitialMoneyByRace(selectedRace['分類']);

        playerData.money = initialMoney;

        // ステータスとアビリティを計算
        const statsRace= calculateStats(selectedRace, raceLevel);
        const statsClass = calculateStats(selectedClass, classLevel);

        console.log("statsRace :", statsRace)
        console.log(" statsClass :", statsClass)

        // 種族データをRole[0]に設定
        playerData.Role[0] = {
            roleName: selectedRace['名前'], // 種族名
            Lv: raceLevel, // 種族レベル
            Ef: 0
        };

        console.log('Role[0]に設定されたデータ:', playerData.Role[0]);

        // クラスデータをRole[1]に設定
        playerData.Role[1] = {
            roleName: selectedClass['名前'], // クラス名
            Lv: classLevel, // クラスレベル
            Ef: 0
        };

        console.log('Role[1]に設定されたデータ:', playerData.Role[1]);

        // 最終的なRole配列を確認
        console.log('最終的なRole配列 (playerData):', playerData.Role);
        console.log("nameInput:", nameInput.value, nameInput);

        // キャラクター名（入力された名前を使用）
        playerData.name = nameInput.value.trim();
    
        // 種族を設定
        playerData.race = selectedRace['名前']; // 種族名を設定
        // statKeys.forEach(key => {
        //     playerData.stats.baseStats[key] = selectedRace[key]/2 || 0; // ボーナス付きのステータス計算
        // });


    } catch (error) {
        // エラー時に詳細をログ出力
        console.error('キャラクターの種族とクラスを設定する際にエラーが発生しました:', error);
    }
}
...
function updateCharacterData() {
    ...
    setCharacterRole(selectedRace, selectedClass, raceLevel, classLevel)
...
confirmLevelsButton.addEventListener('click', async () => {
    ...
    if (currentRaceLevel + currentClassLevel > 10) {
        document.getElementById('message').textContent = 'レベルの合計は10以内である必要があります';
        return;
    }

    console.log(" レベルの合計は10以内である必要があります : ", raceLevelInput, classLevelInput)
    ...
    // 確認ダイアログを表示
    const confirmation = confirm(`以下のキャラクターを作成しますか？\n\n名前: ${nameInput.value.trim()}\n種族: ${selectedRace['名前']}\nクラス: ${selectedClass['名前']}`);
    if (!confirmation) {
    ...
    try {
        ...
        if (response.ok) {
            document.getElementById('message').textContent = `キャラクター「${result.name}」を作成しました！`;
            ...
            document.getElementById('message').textContent = `エラー: ${result.error}`;
            console.error('キャラクター作成エラー:', result.error);
        }
    } catch (error) {
        document.getElementById('message').textContent = 'キャラクター作成に失敗しました';
